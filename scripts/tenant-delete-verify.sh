...
# name + admin email so it can be re-run without colliding with prior runs.
set -euo pipefail

BASE=http://localhost:8080
RUN_ID="$(date +%s)"
TENANT_NAME="PurgeMe-${RUN_ID}"
EMAIL="purge-${RUN_ID}@purge.test"
PASSWORD="purge12345"
...
CREATE=$(curl -fsS -X POST "$BASE/api/platform/tenants" \
  ...
  -H "Content-Type: application/json" \
  -d "{\"name\":\"$TENANT_NAME\",\"industry\":\"qa\",\"initialAdminEmail\":\"$EMAIL\",\"initialAdminPassword\":\"$PASSWORD\"}")
echo "$CREATE"
